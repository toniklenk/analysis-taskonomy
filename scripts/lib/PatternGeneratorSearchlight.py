...
class PatternGeneratorSearchlight(object):
    ...
    def __init__(self, layer_shape: torch.Size, layer_name: str) -> None:
        ...
        self.num_subsets = torch.tensor([dimsize-2 for dimsize in layer_shape]).cumprod(dim=0)[-1]
    ...
    def index_to_mask(self, idx: tuple):
        ...
        mask[idx[0]-1:idx[0]+2,idx[1]-1:idx[1]+2,idx[2]-1:idx[2]+2] = 1
        return mask
    ...
    def __next__(self):
        if self.current_subset < self.num_subsets:
            ...
            idx = PatternGeneratorSearchlight._posnum_to_3Dindex(self.current_subset, self.layer_shape)
    ...
    def get_subset_pattern(self, subset_num: int):
        ...
        return OrderedDict([(
            self.layer_name,
            self.index_to_mask(PatternGeneratorSearchlight._posnum_to_3Dindex(subset_num, self.layer_shape)))])
